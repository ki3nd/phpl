...
class Conv2d(nn.Conv2d, LoRALayer):
    ...
    def init_lora_param(self):
        ...
        # 1. 将四维权重展平为二维矩阵，并升到 float32 做 SVD
        ...
        print(f"权重重建平均绝对误差: {difference.item()}")
        # ----------------------------------------------------
...
class BatchNormLoRA(nn.BatchNorm2d):
    """
    BatchNorm LoRA (BN-LoRA) implementation.
    它不使用低秩分解 (A@B)，而是直接让BatchNorm2d的
    'weight' (gamma) 和 'bias' (beta) 可训练，同时冻结其他参数。
    这通常是 BN 层的 PEFT 最佳实践。
    """
    def __init__(
        self,
        existing_bn: nn.BatchNorm2d,
        r: int = 0, # LoRA rank，这里不使用，但保留接口一致性
        lora_alpha: int = 1, # LoRA alpha，这里不使用，但保留接口一致性
        **kwargs
    ):
        # 1. 初始化 nn.BatchNorm2d
        super().__init__(
            num_features=existing_bn.num_features,
            eps=existing_bn.eps,
            momentum=existing_bn.momentum,
            affine=existing_bn.affine, # 必须为 True 才能有 weight 和 bias
        ...
        # 3. BN-LoRA 的核心逻辑：冻结非训练参数，解冻训练参数
        
        # 冻结所有参数
        for param in self.parameters():
            param.requires_grad = False
            
        # 如果 affine 为 True，则让 weight (gamma) 和 bias (beta) 可训练
        if self.affine:
            if self.weight is not None:
                self.weight.requires_grad = True # <--- BN LoRA 的核心训练参数
            if self.bias is not None:
                self.bias.requires_grad = True # <--- BN LoRA 的核心训练参数
                
        # 冻结 running_mean 和 running_var（这些是 buffer，不参与梯度计算）
        # self.running_mean, self.running_var 已经是 buffer，默认不计算梯度
        # 它们的更新行为取决于 self.training 模式。

        # 4. 保留 LoRA 参数作为兼容性/标记
        self.r = r
        self.lora_alpha = lora_alpha
        self.merged = False # 尽管没有 A/B 合并，但保留状态标记
        
    # 重写 train 方法以控制 running_mean/var 的行为
    def train(self, mode: bool = True):
        super().train(mode)
        
        # 在训练模式下，我们希望 BN-LoRA 冻结统计信息（冻结 running_mean/var 的更新）。
        # 这对于 PEFT 是常见的做法，以防止少量样本对全局统计信息造成污染。
        # 默认情况下，PyTorch BN 在 self.train() 且 track_running_stats=True 时会更新统计量。
        # 显式地将 self.track_running_stats 设为 False (如果它本来是 True)，
        # 可以在训练模式下冻结 running_mean/var 的更新。
        # NOTE: 我们假设 downsample 处的 BN 在预训练时是跟踪统计量的 (track_running_stats=True)。
        # 如果需要冻结统计量，应在初始化时设置或在这里覆盖其行为。
        
        # 更好的做法是在前向传播中控制行为，或在父类中重置统计跟踪：
        # 这里仅确保 weight 和 bias 保持可训练状态。

    def forward(self, x: torch.Tensor):
        # BN-LoRA 在 forward 中没有额外的计算，它只是让 gamma/beta 可训练。
        # 在训练模式下，它使用 mini-batch 统计信息进行归一化（如果 track_running_stats=False）。
        # 在评估模式下，它使用冻结的 running_mean/var。
