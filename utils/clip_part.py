...
def load_clip_to_cpu(cfg, teacher=False):
    if teacher:
        ...
        backbone_name = getattr(trainer_cfg, "TEACHER_NAME", cfg.MODEL.BACKBONE.NAME)
    else:
    ...
    def transformer_deep(self, x, deep_x):
        if self.deep:
            if deep_x.dim == 2:
                if self.deep_layer == None:
                    deep_x = deep_x.unsueeze(0).expand(self.num_layer - 1, -1, -1)  # all layers exist
                else:
                    deep_x = deep_x.unsueeze(0).expand(self.deep_layer[1] - self.deep_layer[0] + 1, -1, -1) # only specified layers exist
        ...
        for i in range(self.num_layer):
            if i == 0:
                ...
                if self.deep:
                    if self.deep_layer == None:   # only specified layers exist
                        if i <= deep_x.shape[0]:
...
class ImageEncoder_Conv(nn.Module):
    ...
    def forward(self, x, vctx=None, deep_vctx=None, return_feat=False):
        '''
        Maybe futherly there will be some tuning methods for convolutional networks
        '''
        def stem(x):
...
class ImageEncoder_Trans(nn.Module):
    ...
    def incorporate_prompt(self, x, vctx):
        # combine learned prompts with image-patch embeddings
