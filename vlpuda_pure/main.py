...
from utils.tools import str2bool, AverageMeter, save_model, ema_update_teacher
...
def get_parser():
    ...
    # teacher_model (models/make_model.py) is hard-copied from base_network
    # at init and otherwise never updated in the original codebase (only
    # read by --rst) -- this experiment gives it a real EMA update.
    parser.add_argument('--teacher_ema', default=False, action='store_true')
    parser.add_argument('--teacher_ema_momentum', type=float, default=0.99)
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, scheduler, args):
    ...
    for e in range(1, args.n_epoch+1):
        ...
        for _ in tqdm(iterable=range(n_batch),desc=f"Train:[{e}/{args.n_epoch}]"):
            ...
            if args.teacher_ema:
                ema_update_teacher(model.teacher_model, model.base_network, args.teacher_ema_momentum)
