...
def ema_update_teacher(teacher_model, base_network, momentum=0.99):
    """EMA (temporal fusion) update for TransferNet.teacher_model, hard-copied
    from base_network at init (models/make_model.py) but never updated
    afterward in the original codebase -- teacher_model is otherwise only
    read by --rst (models/rst.py, utils/tools.py's save_model). Generic
    parameter+buffer-wise EMA over the whole module (not just .model.visual)
    so it stays a structurally-consistent deepcopy of base_network; any
    non-floating buffer (e.g. BatchNorm's num_batches_tracked, if the
    backbone uses one) is hard-copied instead of EMA'd since a float
    momentum update doesn't apply to an integer counter."""
