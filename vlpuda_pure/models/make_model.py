...
class TransferNet(nn.Module):
    def __init__(self, args, train=True):
        ...
        if getattr(args, "init_from", None):
            # Sanity-check hook (PHPL_momentum's vlpuda-sanity branch): load a
            # merged (LoRA-baked-in) CLIP state_dict in place of the vanilla
            # pretrained one -- e.g. an already target-domain-adapted teacher.
            state_dict = torch.load(args.init_from, map_location="cuda")
            missing, unexpected = self.base_network.model.load_state_dict(state_dict, strict=False)
            print(f"[init_from] loaded {args.init_from} "
                  f"(missing={len(missing)}, unexpected={len(unexpected)})")
            # CLIP.__init__ (models/backbone.py) precomputed self.text_features
            # from the OLD (pretrained) text encoder before this override --
            # recompute now, or the zero-shot/"clip" branch would pair NEW image
            # embeddings with STALE text embeddings.
            text_features = self.base_network.encode_text().detach()
            self.base_network.text_features = text_features / text_features.norm(dim=1, keepdim=True)
