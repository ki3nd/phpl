...
from utils.gsl import gradient_scale_layer
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, index+1, attn_mask) for index in range(layers)])
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, vctx=None):
    ...
    def incorporate_prompt(self, x, vctx):
        ...
        x = torch.cat((
            x[:, :1, :],
            vctx.expand(x.shape[0], -1, -1).half(),
            x[:, 1:, :]
