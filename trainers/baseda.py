...
class BaseDA(TrainerXU):
    ...
    def model_backward_and_update_with_gradient_monitoring(
        self, loss, names=None, max_norm=20.0, monitor_interval=10, clip_on_explosion=True
    ):
        """Like model_backward_and_update, but also tracks/clips the gradient norm."""
        ...
        if params:
            if clip_on_explosion:
                ...
                total_norm = torch.norm(torch.stack([p.grad.detach().norm() for p in params]))
        else:
